		...
		<view class="row">
			...
			>
				...
				<view class="mid">
					...
					<view class="bottom">
						<text class="price text-rf">{{ moneySymbol }}{{ productInfo.product_money }} * {{ productInfo.num }}</text>
					</view>
				</view>
			</view>
		</view>
		<view class="product-textarea">
			<form @submit="handleOrderRefundApply">
				...
				<view class="feedback-title">
					<text>退款金额</text>
					<text class="text-rf">{{ moneySymbol }}{{ productInfo.product_money }}</text>
				</view>
							...
							<view class="uni-uploader__files">
								<block v-for="(image, index) in imageList" :key="index">
									<view class="uni-uploader__file" style="position: relative;">
										...
										<view class="close-view bg-rf" @tap="close(index)">x</view>
									</view>
								</block>
				...
				<button class="confirm-btn bg-rf" formType="submit">
...
export default {
	...
	methods: {
		// 数据初始化
		initData() {
			this.productInfo = Json.testData;
			this.refundType = "2";
			let title = '仅退款';
