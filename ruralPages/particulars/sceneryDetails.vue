<template>
	<view class="particulars">
		...
		<view class="detailsMain">
			<view class="title">
				{{ partData.sceneryName }}
			</view>
			<view class="introduce">
				<view class="introduce_item">
					{{ partData.sceneryLabelOne }}
				</view>
				<view v-if="partData.sceneryLabelTwo" class="introduce_item">
					{{ partData.sceneryLabelTwo }}
				</view>
			</view>
			<view class="site">
				<view class="siteInfo">
				...
				<view class="Notice cItem">
					...
					<view class="userNotice">
						...
						<view class="facility">
							...
							<view class="itemList">
								<view class="facilItem" v-for="(item,index) in partData.sceneryFacilities" :key="index">
									{{ item }}
								</view>
							</view>
						</view>
							...
							<view class="cell-vessel">
								<view class="cell_item">
									...
									<view class="cell_value">
										{{ partData.sceneryIndependenttravel }}
									</view>
								</view>
								<view class="cell_item">
									<view class="cell_label">
...
<script>
	...
	export default {
		...
		methods: {
			...
			getDetail() {
				let params = {
					sceneryId: this.parId
				}
				getSceneryDetail(params)
					.then(res => {
						if (res.data.code == '1') {
							let data = JSON.parse(JSON.stringify(res.data.data))
							data.scenery.sceneryFacilities = JSON.parse(data.scenery.sceneryFacilities)
							data.scenery.sceneryCoordinate = JSON.parse(data.scenery.sceneryCoordinate)
							this.partData = data.scenery
							this.teletext = data.sceneryDescribeList
